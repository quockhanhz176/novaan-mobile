...
const signIn = async (usernameOrEmail: string, password:string): Promise<SignInResponse> => {
...
const signUp = async (usernameOrEmail: string, password:string): Promise<SignInResponse> => {
    return await baseApi.post<SignInRequest, SignInResponse>(SIGNIN_URL, {
        usernameOrEmail,
        password,
    });
